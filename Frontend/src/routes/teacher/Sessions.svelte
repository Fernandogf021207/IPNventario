            ...
            <tr>
              ...
              <td class="actions">
                {#if session.status === 'planned'}
                  <button class="btn-icon text-success" on:click={() => confirmStatusChange(session, 'open')} title="Iniciar Sesión">
                ...
                {:else if session.status === 'open'}
                  <button class="btn-icon text-accent" on:click={() => window.location.hash = '#/teacher/attendance'} title="Ver Asistencia">
                    <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M16 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"></path><circle cx="9" cy="7" r="4"></circle><polyline points="16 11 18 13 22 9"></polyline></svg>
                  </button>
                  <button class="btn-icon text-error" on:click={() => confirmStatusChange(session, 'close')} title="Finalizar Sesión">
